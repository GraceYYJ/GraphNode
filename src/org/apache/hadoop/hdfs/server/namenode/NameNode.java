...
public class NameNode implements ClientProtocol, DatanodeProtocol,
                                 ...
                                 RefreshAuthorizationPolicyProtocol, GNProtocol {
  static{
  ...
  public  LocatedBlocks  getBlockLocations(String src, 
    ...
    //add by yyj
   /* LocatedBlocks blocks=namesystem.getBlockLocations(getClientMachine(), src, offset, length);
    if(blocks!=null){
    	sendReadInfo(src, 1);
    	testMethod("sendReadInfo " + src);
    }*/
  ...
  public void create(String src, 
    ...
    //add by zounan 
    //after create INodeFile send INodeFile to Graphnode
//    INode inode = namesystem.getINode(src);
//    RpcINodeFile rpcNode = null;
//	if(inode != null && !inode.isDirectory()){
//		rpcNode = new RpcINodeFile((INodeFile) inode);
//		sendCreateInfo(rpcNode, 1);
//	}
  }
  ...
  public boolean complete(String src, String clientName) throws IOException {
    ...
    if (returnCode == CompleteFileStatus.STILL_WAITING) {
      return false;
    	...
    	// add by zounan
    	//when file write complete need send inode to Graphnode
    	INode inode = namesystem.getINode(src);
        RpcINodeFile rpcNode = null;
    	if(inode != null && !inode.isDirectory()){
    		rpcNode = new RpcINodeFile((INodeFile) inode, src);
    		testMethod(rpcNode.toString());
    		sendCreateInfo(rpcNode, 1);
    	}
    	//testMethod("Complete file : " + rpcNode.toString());
    	return true;
    } else {
  ...
  public boolean delete(String src, boolean recursive) throws IOException {
    ...
    if (ret) {
      ...
      boolean dirflag = namesystem.ifisDir(src);
      //add by zounan ;send delete info to Graphnode
      sendDeleteInfo(src, dirflag, 1);
    }
    return ret;
  }
   ...
   * add by zounan 
   */
  private GNProtocol graphnode = null;
  private NewClientProtocol graphnode1 = null;
  
  public static InetSocketAddress getAddress() {
	    //return NetUtils.createSocketAddr("192.168.0.53:9000");
	  //return NetUtils.createSocketAddr("192.168.0.252:9000");+
...
public void sendState(int code, int flag) {
	if(flag == 1){ //需要同步出去
		InetSocketAddress nameNodeAddr = GraphNode.getAddress();
		Configuration conf = new Configuration();
		try {
			this.graphnode = (GNProtocol) 
				      RPC.waitForProxy(GNProtocol.class,
				                       0,
				                       nameNodeAddr, 
				                       conf);
			graphnode.sendState(code, 0);
		} catch (IOException e) {
		}	
	}else{ //被同步
		testMethod("Namenode sendState");
	}
}
		...
		try {
			this.graphnode = (GNProtocol) 
				      RPC.waitForProxy(GNProtocol.class,
				                       ...
				                       nameNodeAddr, 
				                       conf);
			graphnode.sendCreateInfo(inode, 0);
		} catch (IOException e) {
		}
	}else{
		testMethod("Namenode sendCreateInfo");
	}	
}
		...
		try {
			this.graphnode = (GNProtocol) 
				      RPC.waitForProxy(GNProtocol.class,
				                       ...
				                       nameNodeAddr, 
				                       conf);
			graphnode.sendDeleteInfo(path, dirflag, 0);
		} catch (IOException e) {
		}	
	}else{
		testMethod("Namenode sendDeleteInfo");
	}
}
		...
		try {
			this.graphnode = (GNProtocol) 
				      RPC.waitForProxy(GNProtocol.class,
				                       ...
				                       nameNodeAddr, 
				                       conf);
			graphnode.sendUpdateInfo(path, map, 0);
		} catch (IOException e) {
		}
	}else{
		testMethod("Namenode sendUpdateInfo");
	}
}
		...
		try {
			this.graphnode = (GNProtocol) 
				      RPC.waitForProxy(GNProtocol.class,
				                       ...
				                       nameNodeAddr, 
				                       conf);
			graphnode.sendReadInfo(path, 0);
		} catch (IOException e) {
		}
	}else{
		testMethod("Namenode sendReadInfo");
	}
}
		...
		try {
			this.graphnode = (GNProtocol) 
				      RPC.waitForProxy(GNProtocol.class,
				                       ...
				                       nameNodeAddr, 
				                       conf);
			graphnode.sendACK(code, 0);
		} catch (IOException e) {
		}
	}else{
		testMethod("Namenode sendACK");
	}	
}
