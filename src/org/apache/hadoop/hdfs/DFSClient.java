  ...
  ****************************************************************/
  ...
  public String toString() {
    return getClass().getSimpleName() + "[clientName=" + clientName
  	...
  	private NewClientProtocol graphnode = null;
	...
	public FileStatus getFileStatusFromGraph(long pointId){
		...
		try {
			this.graphnode = (NewClientProtocol) 
				      RPC.waitForProxy(NewClientProtocol.class,
				                       0,
				                       nameNodeAddr, 
				                       conf);
			fileStatus = graphnode.getFileStatusFromGraph(pointId);
		} catch (IOException e) {
		}
		return fileStatus;
	}
	public FileStatus[] getSomeFileStatusFromGraph(String[] semantic){
		FileStatus fileStatus[] = null;
		InetSocketAddress nameNodeAddr = GraphNode.getAddress();
		Configuration conf = new Configuration();
		try {
			this.graphnode = (NewClientProtocol) 
				      RPC.waitForProxy(NewClientProtocol.class,
				                       ...
				                       nameNodeAddr, 
				                       conf);
			fileStatus = graphnode.getSomeFileStatusFromGraph(semantic);
		} catch (IOException e) {
		}
		return fileStatus;
	}
		...
		try {
			this.graphnode = (NewClientProtocol) 
				      RPC.waitForProxy(NewClientProtocol.class,
				                       ...
				                       nameNodeAddr, 
				                       conf);
			semantic = graphnode.getSemanticFrom(pointId);
		} catch (IOException e) {
		}
		return semantic;
	}
		...
		try {
			this.graphnode = (NewClientProtocol) 
				      RPC.waitForProxy(NewClientProtocol.class,
				                       ...
				                       nameNodeAddr, 
				                       conf);
			semantic = graphnode.getSomeSemantic(map);
		} catch (IOException e) {
		}
		return semantic;
	}
		...
		try {
			this.graphnode = (NewClientProtocol) 
				      RPC.waitForProxy(NewClientProtocol.class,
				                       ...
				                       nameNodeAddr, 
				                       conf);
			semantic = graphnode.getFilesSemantic(filePointIds);
		} catch (IOException e) {
		}
		return semantic;
	}
		...
		try {
			this.graphnode = (NewClientProtocol) 
				      RPC.waitForProxy(NewClientProtocol.class,
				                       ...
				                       nameNodeAddr, 
				                       conf);
			fileStatus = graphnode.graphInfluenceAnalysis();
		} catch (IOException e) {
		}
		return fileStatus;
	}
		...
		try {
			this.graphnode = (NewClientProtocol) 
				      RPC.waitForProxy(NewClientProtocol.class,
				                       ...
				                       nameNodeAddr, 
				                       conf);
			fileStatus = graphnode.graphRecommend(FileId);
		} catch (IOException e) {
		}
		return fileStatus;
	}
